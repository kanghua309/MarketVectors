...
class RNNModel():
    def __init__(self):
        ...
        self.input_data = tf.placeholder(dtype=tf.float32, shape=[BATCH_SIZE, num_features],name = "input-data")        #其中batchsize 是不是对应demo中的 num_steps ,而 num_feathure才是真正的batch size？
        ...
        self.zero_state = self.gru_cell.zero_state(num_features, tf.float32)
        ...
        self.start_state = tf.placeholder(dtype=tf.float32,                                                            # 不使用dynamic 时，start_state 用于给嵌套cell输入，然后end state 在作为训练结果，送给嵌套cell 在迭代？ 如果使用dynamic_rnn 则还需要它吗？
                                          shape=[num_features, self.gru_cell.state_size ],  # (50*200)
                                          name="state")
        ...
        states_series, current_state = tf.nn.dynamic_rnn(self.gru_cell,                                                 #收敛的好慢 相比自己构造层次
                                                         inputs=tf.expand_dims(self.input_data, -1),                    #shape=(50, 42, 100)
                                                         initial_state=self.start_state,
                                                         time_major=True                                                #如果 inputs 为 (batches, steps, inputs) ==> time_major=False
                                                         )
